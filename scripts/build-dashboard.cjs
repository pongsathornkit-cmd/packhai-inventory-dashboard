...
function summarizeRows(rows, stockSources, shopee, lazada, ktw, indices) {
  ...
  return {
    metadata: {
      ...
      quantityRule:
        "ใช้ field quantity จาก Packhai และ remaining จาก FlowAccount เป็นจำนวนคงเหลือในคลัง โดย FlowAccount ดึงเฉพาะคลัง ซ.เจริญกิจ และคลัง สุขสวัสดิ์เท่านั้น",
