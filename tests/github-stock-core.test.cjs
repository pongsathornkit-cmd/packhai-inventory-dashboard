...
test("parses multi-warehouse website stock update command", () => {
...
test("assistant creates a website stock update action from the same command", () => {
...
test("applies stock update to selected website warehouses only", () => {
  ...
  assert.equal(saved.rows.find((item) => item.warehouseId === 491662).stockSource, "Website Stock");
  assert.equal(saved.rows.find((item) => item.warehouseId === 491661).source, "Website Stock คลัง ซ.เจริญกิจ");
  assert.equal(saved.exportedAt, "2026-06-26T10:00:00.000Z");
});

test("stock update reply tells users to save stock on the website, not sync FlowAccount", () => {
  const parsed = parseStockUpdateCommand(command);
  const reply = formatStockUpdateReply(parsed);

  assert.match(reply, /Website Stock/);
  assert.doesNotMatch(reply, /Sync คลัง FlowAccount/);
});
