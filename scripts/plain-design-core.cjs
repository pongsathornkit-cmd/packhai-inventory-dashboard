...
function buildPlainDesignInitialState({ seed, dashboard, ktwLogistics }) {
  ...
  const products = (seed?.products || []).map((product, index) => {
    ...
    return {
      ...
      purchaseUnitCostRmb: numberValue(product.purchaseUnitCostRmb),
      supplierAmountUsd: numberValue(product.supplierAmountUsd),
      supplierAmountRmb: numberValue(product.supplierAmountRmb),
      unitsPerCarton: numberValue(product.unitsPerCarton),
